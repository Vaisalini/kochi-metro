...
const AIRecommendationPage = () => {
  ...
  const generateWhatIfScenario = (scenarioType, trainId, modifications) => {
    ...
    const trainIndex = modifiedData.trainsets.findIndex(t => t.id === trainId);
      ...
      switch (scenarioType) {
        ...
        case 'emergency_repair':
          modifiedData.trainsets[trainIndex] = {
            ...originalTrain,
            fitness: {
              rolling: { valid: false, expiry: new Date().toISOString().split('T')[0] },
              signal: { valid: false, expiry: new Date().toISOString().split('T')[0] },
              telecom: { valid: originalTrain.fitness.telecom.valid, expiry: originalTrain.fitness.telecom.expiry }
            },
    ...
    const eligibleTrains = modifiedData.trainsets.filter(t => 
      t.fitness.rolling.valid && t.fitness.signal.valid && t.fitness.telecom.valid && t.jobCards.critical === 0
    );
    
    const ineligibleTrains = modifiedData.trainsets.filter(t => 
      !t.fitness.rolling.valid || !t.fitness.signal.valid || !t.fitness.telecom.valid || t.jobCards.critical > 0
    );
    ...
    eligibleTrains.forEach((train, index) => {
      const trainIndex = modifiedData.trainsets.findIndex(t => t.id === train.id);
      if (trainIndex !== -1) {
    ...
    ineligibleTrains.forEach((train, index) => {
      const trainIndex = modifiedData.trainsets.findIndex(t => t.id === train.id);
      if (trainIndex !== -1) {
        modifiedData.trainsets[trainIndex].aiRank = eligibleTrains.length + index + 1;
      }
    });
  ...
  const calculateScenarioImpact = (originalData, modifiedData) => {
    const originalEligible = originalData.trainsets.filter(t => 
      t.fitness.rolling.valid && t.fitness.signal.valid && t.fitness.telecom.valid && t.jobCards.critical === 0
    ).length;
    
    const modifiedEligible = modifiedData.trainsets.filter(t => 
      t.fitness.rolling.valid && t.fitness.signal.valid && t.fitness.telecom.valid && t.jobCards.critical === 0
    ).length;
    
    const eligibleChange = modifiedEligible - originalEligible;
    const capacityImpact = originalEligible > 0 ? (eligibleChange / originalEligible * 100) : 0;
    
    // Count trains that have different properties (more comprehensive comparison)
    const affectedTrainsCount = modifiedData.trainsets.filter(modifiedTrain => {
      const originalTrain = originalData.trainsets.find(t => t.id === modifiedTrain.id);
      if (!originalTrain) return true;
      
      return (
        modifiedTrain.status !== originalTrain.status ||
        modifiedTrain.fitness.rolling.valid !== originalTrain.fitness.rolling.valid ||
        modifiedTrain.fitness.signal.valid !== originalTrain.fitness.signal.valid ||
        modifiedTrain.fitness.telecom.valid !== originalTrain.fitness.telecom.valid ||
      ...
      <main className="ai-main">
          ...
          {planGenerated && (
            <div className="card" style={{ marginTop: 'var(--space-20)' }}>
              <div className="card__body">
                ...
                <p style={{ fontSize: '14px', color: 'var(--color-text-secondary)', marginBottom: 'var(--space-16)' }}>
                  Create scenarios to analyze the impact of sudden changes on your induction plan
                </p>
                ...
                {whatIfScenarios.length > 0 && (
                  <div style={{ marginTop: 'var(--space-16)' }}>
                    <div style={{ fontSize: '12px', color: 'var(--color-text-secondary)', marginBottom: 'var(--space-8)' }}>
                      <strong>Active Scenarios: {whatIfScenarios.length}</strong>
                    </div>
                    <div className="scenarios-summary">
                      {whatIfScenarios.map(scenario => (
                        <div key={scenario.id} className="scenario-summary-item">
                          <span className="scenario-summary-name">{scenario.name}</span>
        ...
        <div className="ai-results">
                    ...
                    {appData.conflicts.map(conflict => (
                      <div key={conflict.id} className="conflict-item">
                        ...
                        {resolvedConflicts.includes(conflict.id) ? (
                          <div style={{color: '#21808d', fontWeight: 'bold', marginTop: 8}}>Conflict Resolved</div>
                        ) : (
          ...
          {planGenerated && whatIfScenarios.length > 0 && (
            <div className="what-if-section" style={{ marginTop: 'var(--space-32)' }}>
              <div className="card">
                <div className="card__body">
                    ...
                    {whatIfScenarios.map(scenario => (
                      <div key={scenario.id} className="scenario-item">
                        <div className="scenario-header">
                          <h5>{scenario.name}</h5>
                          <span className="scenario-timestamp">{scenario.timestamp}</span>
                        </div>
                        <p className="scenario-description">{scenario.description}</p>
                        
                        <div className="scenario-impact">
                          <div className="impact-metric">
                            <strong>Service Capacity Impact:</strong> {scenario.impact.serviceCapacityImpact}%
                          </div>
                          <div className="impact-metric">
                            <strong>Eligible Trains Change:</strong> {scenario.impact.eligibleTrainsChange > 0 ? '+' : ''}{scenario.impact.eligibleTrainsChange}
                          </div>
                          <div className="impact-metric">
                            <strong>Affected Trains:</strong> {scenario.impact.affectedTrains}
                          </div>
                        </div>
                        ...
                        <div className="scenario-actions">
                          ...
                          >
                            {currentScenario?.id === scenario.id ? 'Currently Active' : 'View Scenario Plan'}
                          </button>
                        ...
                        {currentScenario?.id === scenario.id && (
                          <div className="scenario-plan" style={{ marginTop: 'var(--space-20)' }}>
                            <div className="scenario-plan-header" style={{ 
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-8)' }}>
                                ...
                                <strong>Scenario Impact - Updated Induction Plan</strong>
                              </div>
                            </div>

                            <div className="updated-plan-list">
                              <h6 style={{ marginBottom: 'var(--space-12)', color: 'var(--color-text)' }}>
                                Updated Train Ranking for "{scenario.name}"
                              </h6>
                              {[...scenario.modifiedData.trainsets].sort((a, b) => a.aiRank - b.aiRank).map((train, index) => (
                                <div key={train.id} className={`plan-item ${train.id === scenario.trainId ? 'plan-item--affected' : ''}`}>
                                  <div>
                                    <div className="plan-rank">#{train.aiRank}</div>
                                    <div className="plan-train">{train.id}</div>
                                    <div className="plan-reasoning">
...
const WhatIfScenarioModal = ({ isOpen, onClose, onGenerateScenario, trains }) => {
  const [selectedScenarioType, setSelectedScenarioType] = useState('fc_cancelled');
  const [selectedTrainId, setSelectedTrainId] = useState('');
  const [customModifications, setCustomModifications] = useState({});

  const scenarioTypes = [
    { value: 'fc_cancelled', label: 'Fitness Certificate Cancelled', description: 'Remove train from service due to FC cancellation' },
    { value: 'critical_maintenance', label: 'Critical Maintenance Required', description: 'Add critical maintenance requirement' },
    { value: 'emergency_repair', label: 'Emergency Repair', description: 'Multiple systems failure requiring immediate repair' },
    { value: 'standby_activation', label: 'Activate Standby Train', description: 'Bring standby train into revenue service' },
    { value: 'custom', label: 'Custom Modification', description: 'Apply custom modifications to train status' }
  ];
  ...
  const handleGenerate = () => {
    ...
    onGenerateScenario(selectedScenarioType, selectedTrainId, customModifications);
    setSelectedTrainId('');
    setCustomModifications({});
  };

  const availableTrains = selectedScenarioType === 'standby_activation' 
      ...
      <div style={{ minWidth: '500px' }}>
        <div className="form-group">
          ...
          <small style={{ color: 'var(--color-text-secondary)', marginTop: '4px', display: 'block' }}>
            {scenarioTypes.find(t => t.value === selectedScenarioType)?.description}
          </small>
        </div>
          ...
          >
            ...
            {availableTrains.map(train => (
              <option key={train.id} value={train.id}>
                {train.id} - {train.status} (Rank #{train.aiRank})
              </option>
            ))}
          </select>
        </div>
        ...
        {selectedScenarioType === 'custom' && (
          <div className="form-group">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 'var(--space-12)' }}>
              <div>
                <label style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>New Status</label>
                <select 
                  className="form-control"
                  onChange={(e) => setCustomModifications(prev => ({ ...prev, status: e.target.value }))}
                >
              ...
              <div>
                <label style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>Add Critical Jobs</label>
                <input 
                  type="number" 
                  className="form-control"
                  placeholder="0"
                  min="0"
                  onChange={(e) => setCustomModifications(prev => ({ 
                    ...prev, 
                    jobCards: { 
                      ...prev.jobCards, 
        ...
        <div style={{ 
          ...
          <strong>Preview:</strong> This scenario will simulate {scenarioTypes.find(t => t.value === selectedScenarioType)?.description.toLowerCase()} 
          {selectedTrainId && ` for train ${selectedTrainId}`}. 
          The system will recalculate the induction plan and show the impact on service capacity.
        </div>
      </div>
    </Modal>
  );
